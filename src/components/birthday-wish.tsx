...
const candleColors = ['#FF0000', '#FFA500', '#FFFF00', '0000FF', '#FE11A5']
const balloonColors = ['#FF0000', '#FFA500', '#FFFF00', '0000FF', '#FE11A5']
const confettiColors = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#FFA07A', '#98D8C8', '#F7DC6F', '#BB8FCE', '#FF0000', '#FFA500', '#FFFF00', '0000FF', '#FE11A5']
      ...
      >
        ...
        <Card className="mx-auto overflow-hidden transition-all duration-300 ease-in-out hover:shadow-xl border-2 border-black">
          ...
          <CardHeader className="text-center">
            <CardTitle className="text-4xl font-bold text-red-400">Happy Birthday!</CardTitle>
            <CardDescription className="text-2xl font-semibold text-amber-600">Hafsah Imran</CardDescription>
            <p className="text-lg text-amber-500">October 17th</p>
          </CardHeader>
          {/* Card content with candles and balloons */}
          <CardContent className="space-y-6 text-center">
            {/* Candles section */}
            <div>
              <h3 className="text-lg font-semibold text-orange-600 mb-2">Light the candles:</h3>
                ...
                {[...Array(totalCandles)].map((_, index) => (
                  <AnimatePresence key={index}>
                    ...
                    {(celebrating && index <= candlesLit) || (!celebrating && index < candlesLit) ? (
                      ...
                      // Unlit candle
                      <FaBirthdayCake
                        className={`w-8 h-8 text-gray-400 transition-colors duration-300 ease-in-out cursor-pointer hover:scale-110`}
                        onClick={() => lightCandle(index)}
                      />
                    )}
                  </AnimatePresence>
          ...
          <CardFooter className="flex justify-center">
            <Button 
              className="bg-gray-700 text-yellow-100 hover:bg-blue-800 transition-all duration-300"
              onClick={celebrate}
